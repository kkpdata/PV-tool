...
def add_proefresultaten_su_sv(self: SHANSEP):
    """Deze functie voegt de proefresultaten toe aan de figuur."""
    boring_monsternummer = self.shansep_data_df_oc.index

    x_proefresultaten = self.shansep_data_df_oc['S\'']
    y_proefresultaten = self.shansep_data_df_oc['T']
...
def get_extra_data(self: SHANSEP, investigationgroups_extra: Optional[List]):
    if self.analysis_type in ['TXT_shansep', 'TXT_SH']:
        dataset_df = self.dbase_df[self.dbase_df['ALG__TRIAXIAAL']]
    elif self.analysis_type in ['DSS_shansep', 'DSS_SH']:
    ...
    if self.analysis_type in ['DSS_shansep', 'DSS_SH']:
...
def add_5pr_bovengrens(self: SHANSEP):
    """Deze functie voegt de 5% bovengrens toe aan de figuur."""
    x_5pr = self.shansep_analyses_data_df['s\'']
    y_5pr = self.shansep_analyses_data_df['5pr_bovengrens_cor']
...
def add_raaklijn_kar_boven(self: SHANSEP):
...
def add_5pr_ondergrens(self: SHANSEP):
    """Deze functie voegt de 5% bovengrens toe aan de figuur."""
    x_5pr = self.shansep_analyses_data_df['s\'']
    y_5pr = self.shansep_analyses_data_df['5pr_ondergrens_cor']
...
def add_raaklijn_kar_onder(self: SHANSEP):
...
def add_fysische_realiseerbare_ondergrens(self: SHANSEP):
    """Deze functie voegt de fysische realiseerbare ondergrens toe aan de figuur."""
    raaklijn_kar_x1 = 0
    raaklijn_kar_x2 = self.shansep_analyses_data_df['S\''].max() + 5
