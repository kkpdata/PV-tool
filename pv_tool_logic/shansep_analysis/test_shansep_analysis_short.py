...
def run_calculations_and_figures(analyse: SHANSEP, analysis_type: str):
    ...
    try:
        ...
        analyse.save_fig_html(
            path=str(export_dir), fig=fig_sv_su, export_name=f"test_shansep_sv_su_{analysis_type}.html"
        )
        fig_sv_su_nc = analyse.show_figure_sv_su_nc()
        analyse.save_fig_html(
            path=str(export_dir), fig=fig_sv_su_nc, export_name=f"test_shansep_sv_su_nc_{analysis_type}.html"
        )
        fig_ln_ocr_ln_s = analyse.show_figure_ln_ocr_ln_s()
        analyse.save_fig_html(
            path=str(export_dir), fig=fig_ln_ocr_ln_s, export_name=f"test_shansep_ln_ocr_ln_s_{analysis_type}.html"
        )
    except Exception as e:
