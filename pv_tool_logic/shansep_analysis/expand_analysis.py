...
def calculate_pop(self: SHANSEP):
    self.shansep_data_df.loc[:, "POP"] = (
        self.shansep_data_df["terreinspanning"] * self.shansep_data_df["OCR"]
...
def calculate_5pr_ondergrens_oc(self: SHANSEP):
    formule = (
        e_a1_oc(self)
        + e_a2_oc(self) * self.shansep_data_df_oc["s'"]
        - t_n_2_oc(self)
        * (
            sigma_a1_oc(self) ** 2
            + self.shansep_data_df_oc["s'"] ** 2 * sigma_a2_oc(self) ** 2
            + 2 * rho_a1_a2_oc(self) * self.shansep_data_df_oc["s'"] * sigma_a1_oc(self) * sigma_a2_oc(self)
            + (1.0 - self.alpha) * (sum_chi_2_oc(self) / (count_sv_oc(self) - 2))
        )
        ** 0.5
    )
...
def calculate_5pr_bovengrens_oc(self: SHANSEP):
    formule = (
        ...
        * (
            ...
            + self.shansep_data_df_oc["s'"] ** 2 * sigma_a2_oc(self) ** 2
            + 2 * rho_a1_a2_oc(self) * self.shansep_data_df_oc["s'"] * sigma_a1_oc(self) * sigma_a2_oc(self)
            + (1 - self.alpha) * (sum_chi_2_oc(self) / (count_sv_oc(self) - 2))
        )
        ** 0.5
    )
...
def calculate_sv_ty_ondergrens_oc(self: SHANSEP):
    formule = (self.shansep_data_df_oc["s'"] - sum_s_eff_oc(self) / count_s_eff_oc(self)) * self.shansep_data_df_oc[
        "5_pr_ondergrens"
    ]
...
def calculate_chi_2_ondergrens_oc(self: SHANSEP):
    formule = (
        self.shansep_data_df_oc["5_pr_ondergrens"] - a1_kar_oc(self) - a2_kar_oc(self) * self.shansep_data_df_oc["s'"]
    ) ** 2
...
def calculate_chi_2_nc_oc(self: SHANSEP):
    formule = (
        self.shansep_data_df_nc_oc["LN(su/svc)"]
        - e_a1_nc_oc(self)
        - e_a2_nc_oc(self) * self.shansep_data_df_nc_oc["LN(OCR)"]
    ) ** 2
...
def calculate_5pr_ondergrens_nc_oc(self: SHANSEP):
    formule = (
        e_a1_nc_oc(self)
        + e_a2_nc_oc(self) * self.shansep_data_df_nc_oc["s'"]
        - t_n_2_nc_oc(self)
        * (
            sigma_a1_nc_oc(self) ** 2
            + self.shansep_data_df_nc_oc["s'"] ** 2 * sigma_a2_nc_oc(self) ** 2
            + 2 * rho_a1_a2_nc_oc(self) * self.shansep_data_df_nc_oc["s'"] * sigma_a1_nc_oc(self) * sigma_a2_nc_oc(self)
            + (1.0 - self.alpha) * (sum_chi_2_nc_oc(self) / (count_ln_ocr_nc_oc(self) - 2))
        )
        ** 0.5
    )
...
def calculate_5pr_bovengrens_nc_oc(self: SHANSEP):
    formule = (
        ...
        * (
            ...
            + self.shansep_data_df_nc_oc["s'"] ** 2 * sigma_a2_nc_oc(self) ** 2
            + 2 * rho_a1_a2_nc_oc(self) * self.shansep_data_df_nc_oc["s'"] * sigma_a1_nc_oc(self) * sigma_a2_nc_oc(self)
            + (1 - self.alpha) * (sum_chi_2_nc_oc(self) / (count_ln_ocr_nc_oc(self) - 2))
        )
        ** 0.5
    )
...
def calculate_chi_2_ondergrens_nc_oc(self: SHANSEP):
    formule = (
        self.shansep_data_df_nc_oc["5_pr_ondergrens"]
        - a1_kar_nc_oc(self)
        - a2_kar_nc_oc(self) * self.shansep_data_df_nc_oc["s'"]
    ) ** 2
