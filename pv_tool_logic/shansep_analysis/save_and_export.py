...
def _create_input_table(self: "SHANSEP") -> Table:
    ...
    if (
        hasattr(self, "shansep_data_df_nc_oc_unsorted")
        and self.shansep_data_df_nc_oc_unsorted is not None
        and len(self.shansep_data_df_nc_oc_unsorted) == len(table_df)
    ):
    ...
    if "consolidatietype" in table_df.columns:
        table_df = table_df.sort_values(
            by=["consolidatietype", table_df.index.name or table_df.index], ascending=[False, True]
        ).copy()
    else:
...
def save_to_pdf(self: "SHANSEP", path: str) -> str:
    ...
    file_name = (
        f"shansep_pdf_export_{self.investigation_groups[0]}_{self.analysis_type}_"
    ...
    try:
        ...
        print(
            f"Waarschuwing: Kon tweede figuur (ln(OCR) - ln(s<sub>u</sub>/σ'<sub>v</sub>)) "
    ...
    try:
        ...
        if hasattr(self, "figure_sv_su_nc") and self.figure_sv_su is not None:
            ...
            story.append(
                Paragraph("SU-tabel (σ'<sub>v</sub> - s<sub>u</sub> relatie) op alleen NC proeven", styles["Heading2"])
            )
