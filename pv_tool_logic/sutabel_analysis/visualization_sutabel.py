...
def add_extra_proefresultaten_ln_sv_ln_su_sutabel(self: "SUTABEL", extra_groepen: Optional[List]):
    ...
    df.loc[:, "ln(s'v)"] = df["S'v"].apply(lambda x: np.log(x) if x is not None and x > 0 else "")

    df.loc[:, "ln(su)"] = df["Su"].apply(lambda x: np.log(x) if x is not None and x > 0 else "")
...
def add_linear_fit_ln_sv_ln_su_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import e_a2_sutabel, e_a1_sutabel
...
def add_fysische_realiseerbare_ondergrens_ln_sv_ln_su_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import a2_kar_sutabel, a1_kar_sutabel
    ...
    self.figure_ln_sv_ln_su.add_trace(
        go.Scatter(x=x, y=y, mode="lines", name="Fysische realiseerbare ondergrens", line=dict(color="black", width=2))
    )
...
def add_su_kar_fit_constante_vc(self: "SUTABEL"):
    ...
    self.figure_sv_su.add_trace(
        go.Scatter(
            x=x, y=y, mode="lines", name="Su kar fit met constante vc", line=dict(color="red", width=2, dash="dashdot")
        )
    )
