...
def calculate_ln_sv_sutabel(self: "SUTABEL"):
    ...
    self.sutabel_data_df.loc[:, "ln(s'v)"] = self.sutabel_data_df["S'v"].apply(
...
def calculate_ln_su_sutabel(self: "SUTABEL"):
    ...
    self.sutabel_data_df.loc[:, "ln(su)"] = self.sutabel_data_df["Su"].apply(
        lambda x: np.log(x) if x is not None and x > 0 else ""
    )
...
def calculate_sv_tt_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import count_ln_sv_sutabel, sum_ln_sv_sutabel
...
def calculate_chi_2_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import e_a1_sutabel, e_a2_sutabel

    formule = (
        self.sutabel_data_df["ln(su)"] - e_a1_sutabel(self) - e_a2_sutabel(self) * self.sutabel_data_df["ln(s'v)"]
    ) ** 2
...
def calculate_sv_eff_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import count_ln_sv_sutabel
...
def calculate_5pr_ondergrens_sutabel(self: "SUTABEL"):
    ...
    formule = (
        e_a1_sutabel(self)
        + e_a2_sutabel(self) * self.sutabel_data_df["s'"]
        - t_n_2_sutabel(self)
        * (
            sigma_a1_sutabel(self) ** 2
            + self.sutabel_data_df["s'"] ** 2 * sigma_a2_sutabel(self) ** 2
            + 2 * rho_a1_a2_sutabel(self) * self.sutabel_data_df["s'"] * sigma_a1_sutabel(self) * sigma_a2_sutabel(self)
            + (1.0 - self.alpha) * (sum_chi_2_sutabel(self) / (count_ln_sv_sutabel(self) - 2))
        )
        ** 0.5
    )
...
def calculate_5pr_bovengrens_sutabel(self: "SUTABEL"):
    ...
    formule = (
        ...
        * (
            ...
            + self.sutabel_data_df["s'"] ** 2 * sigma_a2_sutabel(self) ** 2
            + 2 * rho_a1_a2_sutabel(self) * self.sutabel_data_df["s'"] * sigma_a1_sutabel(self) * sigma_a2_sutabel(self)
            + (1 - self.alpha) * (sum_chi_2_sutabel(self) / (count_ln_sv_sutabel(self) - 2))
        )
        ** 0.5
    )
...
def calculate_sv_tt_ondergrens_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import sum_s_eff_sutabel, count_s_eff_sutabel
...
def calculate_sv_ty_ondergrens_sutabel(self: "SUTABEL"):
    ...
    formule = (self.sutabel_data_df["s'"] - sum_s_eff_sutabel(self) / count_s_eff_sutabel(self)) * self.sutabel_data_df[
        "5_pr_ondergrens"
    ]
...
def calculate_chi_2_ondergrens_sutabel(self: "SUTABEL"):
    ...
    from pv_tool_logic.sutabel_analysis.variables import a1_kar_sutabel, a2_kar_sutabel

    formule = (
        self.sutabel_data_df["5_pr_ondergrens"]
        - a1_kar_sutabel(self)
        - a2_kar_sutabel(self) * self.sutabel_data_df["s'"]
    ) ** 2
