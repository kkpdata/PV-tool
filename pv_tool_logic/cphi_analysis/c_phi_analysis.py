...
from pv_tool_logic.cphi_analysis.save_and_export import save_total_to_excel, save_to_pdf

from pv_tool_logic.imports.import_data import Dbase
import plotly.graph_objects as go
from pv_tool_logic.cphi_analysis.expand_analysis_df import (
    calculate_tan_a,
    calculate_ln_tan_a,
    calculate_s_tt,
    calculate_s_ty,
    calculate_kappa_2,
    calculate_s,
    calculate_5pr_ondergrens,
    calculate_5pr_bovengrens,
    calculate_s_tt_ondergrens,
    calculate_s_ty_ondergrens,
    calculate_kappa_2_ondergrens,
    calculate_correctie_t,
    kappa_2_2pr_cor,
...
from pv_tool_logic.cphi_analysis.visualization import (
    add_proefresultaten,
...
class CPhiAnalyse:
    ...
    def get_cphi_data(self):
        ...
        if self.analysis_type in ["TXT_CPhi", "TXT_SH"]:
            ...
            self.cphi_analyses_data_df = self.cphi_analyses_data_df[
                self.cphi_analyses_data_df["PV_NAAM"].isin(self.investigation_groups)
            ]
    ...
    def apply_settings(
        ...
        self.material_cohesie = (
            material_factor_cohesion if material_factor_cohesion is not None else self.material_cohesie
        )
        self.material_tan_phi = (
            material_factor_tan_phi if material_factor_tan_phi is not None else self.material_tan_phi
        )

    def apply_parameters(
        self, cohesie_gem: Optional[float] = None, phi_kar: Optional[float] = None, cohesie_kar: Optional[float] = None
    ):
        """
    ...
    def plot_spanningspaden(self):
            ...
            if stress_data["S'"]:  # Als er data is voor dit monster
                ...
                if self.analysis_type in ["TXT_CPhi", "TXT_SH"]:
                    ...
                    if not isna(rek_bij_t_piek) and "pieksterkte" in stress_df["stress_state"].values:
                        ...
                        stress_df = concat(
                            [stress_df.iloc[:insert_index], piek_row, stress_df.iloc[insert_index:]]
        ...
        if sample_stress_paths:
            from pv_tool_logic.cphi_analysis.visualization import add_stress_paths
