...
from pv_tool_logic.cphi_analysis.variables import *
...
def calculate_5pr_ondergrens(self: CPhiAnalyse):
    ...
    formule = (
        e_a1(self)
        + e_a2(self) * self.cphi_analyses_data_df["s'"]
        - t_n_2(self)
        * (
            sigma_a1(self) ** 2
            + self.cphi_analyses_data_df["s'"] ** 2 * sigma_a2(self) ** 2
            + 2 * rho_a1_a2(self) * self.cphi_analyses_data_df["s'"] * sigma_a1(self) * sigma_a2(self)
            + (1.0 - self.alpha) * (sum_kappa_2(self) / (count_s(self) - 2))
        )
        ** 0.5
    )
...
def calculate_5pr_bovengrens(self: CPhiAnalyse):
    ...
    formule = (
        ...
        * (
            ...
            + self.cphi_analyses_data_df["s'"] ** 2 * sigma_a2(self) ** 2
            + 2 * rho_a1_a2(self) * self.cphi_analyses_data_df["s'"] * sigma_a1(self) * sigma_a2(self)
            + (1 - self.alpha) * (sum_kappa_2(self) / (count_s(self) - 2))
        )
        ** 0.5
    )
...
def calculate_s_ty_ondergrens(self: CPhiAnalyse):
    ...
    formule = (self.cphi_analyses_data_df["s'"] - sum_s2(self) / count_s2(self)) * self.cphi_analyses_data_df[
        "5_pr_ondergrens"
    ]
...
def calculate_kappa_2_ondergrens(self: CPhiAnalyse):
    ...
    formule = (
        self.cphi_analyses_data_df["5_pr_ondergrens"] - a1_kar(self) - a2_kar(self) * self.cphi_analyses_data_df["s'"]
    ) ** 2
...
def kappa_2_2pr_cor(self: CPhiAnalyse):
    ...
    if self.cohesie_gem_handmatig is not None:
        ...
        coh_gem = self.eerste_benadering_a1_gem
    formule = (self.cphi_analyses_data_df["T"] - coh_gem - helling_gecor(self) * self.cphi_analyses_data_df["S'"]) ** 2
...
def calculate_5pr_ondergrens_correctie_c(self: CPhiAnalyse):
    ...
    formule = (
        coh_gem
        + helling_gecor(self) * self.cphi_analyses_data_df["s'"]
        - t_n_2(self)
        * (
            sigma_a1_gecorrigeerd(self) ** 2
            + self.cphi_analyses_data_df["s'"] ** 2 * sigma_a2_gecorrigeerd(self) ** 2
            + 2
            * rho_a1_a2(self)
            * self.cphi_analyses_data_df["s'"]
            * sigma_a1_gecorrigeerd(self)
            * sigma_a2_gecorrigeerd(self)
            + (1 - self.alpha) * (sum_kappa_2_2pr_gecorrigeerd(self) / (count_s(self) - 2))
        )
        ** 0.5
    )
...
def calculate_kappa_2_ondergrens_correctie_c(self: CPhiAnalyse):
    ...
    formule = (
        ...
        - a1_kar(self)
        - a2_kar(self) * self.cphi_analyses_data_df["s'"]
    ) ** 2
