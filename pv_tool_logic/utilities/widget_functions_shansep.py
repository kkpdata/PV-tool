...
from pv_tool_logic.utilities.widget_functions_cphi import maak_verzamelings_lijsten, koppel_callbacks
...
def maak_proef_widgets_shansep(pv_txt_lijst, pv_dss_lijst):
    ...
    return (
        ...
        container_rekpercentage_shansep,
...
def dropdown_widgets_shansep(dbase):
    ...
    (
        ...
        container_rekpercentage_shansep,
        output_rekpercentage_shansep,
    ) = maak_proef_widgets_shansep(PV_txt_lijst, PV_dss_lijst)
...
def run_shansep_analysis(
    ...
    rekpercentage = (
        dropdown_rekpercentage_txt_shansep.value
        if dropdown_type_proef_shansep.value.startswith("TXT")
        else dropdown_rekpercentage_dss_shansep.value
    )
    ...
    if analyse.e_a2_nc_oc > 1:
        print(
            "Waarschuwing: De inschatting van de gemiddelde waarde van m is groter dan 1. "
            "Dit is fysisch niet mogelijk. Overweeg om deze waarde aan te passen."
        )

    if analyse.a2_kar_nc_oc > 1:
        print(
            "Waarschuwing: De inschatting van de karakteristieke waarde van m is groter dan 1. "
            "Dit is fysisch niet mogelijk. Overweeg om deze waarde aan te passen."
        )

    # Ophalen laatste resultaten
    handmatig = get_laatste_resultaten_shansep(
        analyse, import_dropdown, import_filechooser, export_dir_widget, export_name_widget
    )
    handmatig_gem = (handmatig["PV_A1_SNIJPUNT_YAS_GEM"][0], handmatig["PV_A2_S_GEM"][0], handmatig["PV_m_GEM"][0])
    handmatig_kar = (handmatig["PV_A1_SNIJPUNT_YAS_KAR"][0], handmatig["PV_A2_S_KAR"][0], handmatig["PV_m_KAR"][0])
...
def get_laatste_resultaten_shansep(
    ...
    (
        ...
        S_KAR_BENADERING_UIT_S_en_POP_R2,
        m_KAR_BENADERING_uit_S_en_m_R3,
    ) = get_benaderings_waarden(analyse)
...
def show_grids(analyse, handmatig_gem, handmatig_kar):
    ...
    for i, row in enumerate(_create_grid_row(descriptions, *benadering_gem), start=1):
    ...
    for i, row in enumerate(_create_grid_row(descriptions, *benadering_kar), start=1):
        for j, item in enumerate(row):
            grid_kar[i, j] = item
