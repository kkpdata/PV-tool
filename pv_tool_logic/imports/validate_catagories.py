...
def validate_alg(self: Validation):
    ...
    if not self.critical:
        schema = Schema(
            [Column("ALG_NAAM_POLDER_DIJK", [IsEmptyValidator]), Column("ALG_REFERENTIE", [IsEmptyValidator])]
        )
...
def validate_kenmerken_boring(self: Validation):
    ...
    if not self.critical:
        schema = Schema(
            [Column("BORING_FILENAAM_PDF", [IsEmptyValidator]), Column("BORING_FILENAAM_GEF", [IsEmptyValidator])]
        )
    else:
        schema = Schema(
            [
...
def validate_monster(self: Validation):
    ...
    if not self.critical:
        schema = Schema(
            [Column("MONSTER_NIVEAU_MV_VANAF", [IsEmptyValidator]), Column("MONSTER_NIVEAU_MV_TOT", [IsEmptyValidator])]
        )
    else:
        schema = Schema(
            [
...
def validate_clas(self: Validation):
    ...
    if not self.critical:
        ...
        schema = Schema([Column("CLAS_MONSTERID", [IsEmptyValidator])])
    ...
    if not self.critical:
        schema = Schema(
            [
                ...
                Column("SD_GRONDSOORT", [IsEmptyValidator]),
