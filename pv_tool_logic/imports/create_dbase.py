...
from pv_tool_logic.imports.add_ana_columns import add_txt_consol_type_reken, add_dss_consol_type_reken
from pv_tool_logic.imports.globals import (
    PV_TOOL_DBASE_COLUMNS,
    CLAS_COLUMNS,
    CRS_COLUMNS,
    SD_COLUMNS,
    DSS_COLUMNS,
...
from pv_tool_logic.imports.add_ana_columns import (
    add_columns,
    add_terreinspanning,
    add_txt_max_vert_consol_sp,
    add_dss_max_consol_sp,
    add_txt_consol_type,
    add_dss_consol_type,
    add_grensspanning_proef,
    calc_pop_veld,
    calc_pop_average,
    add_grensspanning_voorstel,
    calc_grensspanning_reken,
    calc_ocr_txt,
    calc_ocr_dss,
)
...
def add_pv_naam(self):
    ...
    if self.dbase_df["PV_NAAM"].isnull().all() or (self.dbase_df["PV_NAAM"] == "").all():
        self.dbase_df["PV_NAAM"] = self.dbase_df.apply(
            lambda row: "TXT-proef" if row["ALG__TRIAXIAAL"] else ("DSS-proef" if row["ALG__DSS"] else None), axis=1
        )
